...
const FilDeVie = ({ data, db }) => {
  ...
  // Le fil ondule de gauche à droite avec des courbes de Bézier
  const cx = W / 2;
  const amplitude = 52; // amplitude de l'ondulation

  // Points de passage du fil — un nœud par moment + début/fin
  const nodeY = (i) => 80 + i * ENTRY_H + CARD_H / 2;
  const nodeX = (i) => cx + (i % 2 === 0 ? -amplitude : amplitude);
  ...
  const buildPath = () => {
    ...
    // Fin : retour au centre
    const lastX = nodeX(n - 1);
    const lastY = nodeY(n - 1);
    d += ` C ${(lastX + cx) / 2} ${lastY}, ${(lastX + cx) / 2} ${SVG_H - 40}, ${cx} ${SVG_H - 40}`;
    return d;
  };

  // Côté de la carte (opposé au nœud)
  const cardSide = (i) => i % 2 === 0 ? "left" : "right";
    ...
    <div style={{ padding: "1.5rem 0 8rem", maxWidth: 520, margin: "0 auto", minHeight: "100vh" }}>
        ...
        <div style={{
          ...
          <textarea
            ...
            style={{
              ...
              color: T.aube, fontFamily: T.serif, fontStyle: "italic",
              fontSize: "1rem", resize: "none", outline: "none",
              lineHeight: 1.7, padding: "0.5rem 0",
      ...
      <div style={{ position: "relative", width: "100%", overflow: "hidden" }}>
        ...
        >
          ...
          <path
            d={buildPath()}
            stroke="url(#filGradient)"
            strokeWidth="1.5"
            fill="none"
            filter="url(#glowFil)"
          />
          {/* Fil intérieur brillant */}
          <path
            d={buildPath()}
            stroke={T.or}
            strokeWidth="0.5"
        ...
        {/* ── Cartes posées sur le fil ── */}
        <div style={{ position: "relative", zIndex: 1, height: SVG_H }}>
          {affiche.map((m, i) => {
            const ti = typeInfo(m.type);
            const side = cardSide(i);
            const nodeXpx = `${(nodeX(i) / W * 100).toFixed(1)}%`;
            const yPx = nodeY(i) - CARD_H / 2;
            const isDemo = m.id < 0;
            ...
            return (
              <div
                ...
                style={{
                  ...
                  right: side === "right" ? "2%" : "auto",
                  width: "43%",
                  animation: `fadeUp 0.7s ease forwards ${i * 0.12}s`,
                  opacity: 0,
                }}
              >
                {/* Ligne de connexion au fil (dessinée en CSS) */}
                <div style={{
                  position: "absolute",
                  top: "50%",
                  [side === "left" ? "right" : "left"]: "-12%",
                ...
                <div style={{
                  ...
                  {/* Suppression (appui long simulé par double-clic) */}
                  {!isDemo && (
                    <button
                      onDoubleClick={() => deleteMoment(m.id)}
                      title="Double-cliquer pour supprimer"
                      style={{
                        position: "absolute", top: "0.4rem", right: "0.4rem",
                        background: "none", border: "none", cursor: "pointer",
                        color: T.brume, fontSize: "0.55rem", opacity: 0.3,
                        padding: "0.2rem",
                      }}
                    >✕</button>
                  )}
